...
app.use(session({
    secret: process.env.SESSION_SECRET || "sp-talkies-demo-secret",
    resave: false,
...
app.post("/api/login", (req, res) => {
    ...
    const p = process.env.ADMIN_PASSWORD || "SPTalkies@123";
...
app.get("/api/movies/:id", (req, res) => {
    const m = read().find(x => x.id === Number(req.params.id));
    if (m) {
        return res.json(m);
    }
...
app.post("/api/movies", auth, upload.single("poster"), (req, res) => {
    const a = read();
    const m = {
        ...
        category: req.body.category || "Films",
        description: req.body.description || "",
        poster: req.file ? "/uploads/" + req.file.filename : (req.body.poster || ""),
...
app.put("/api/movies/:id", auth, upload.single("poster"), (req, res) => {
    const a = read();
    ...
    a[i] = {
        ...a[i],
        title: req.body.title || a[i].title,
        year: req.body.year ? +req.body.year : null,
        genre: req.body.genre || "",
        category: req.body.category || "Films",
        description: req.body.description || "",
        poster: req.file ? "/uploads/" + req.file.filename : (req.body.poster || a[i].poster),
        trailer: req.body.trailer || "",
        featured: req.body.featured === "true"
    };
...
app.delete("/api/movies/:id", auth, (req, res) => {
    write(read().filter(x => x.id !== Number(req.params.id)));
    res.json({ ok: true });
});
